...
const heroImages = [
  ...
  "/images/custom-order/carosel/4.JPG"
];
...
const CustomOrderPage = () => {
  ...
  const steps = [
    {
      ...
      description: (
        <span>
          แจ้งงบประมาณ (Budget) <Link href="/material-compare" className="underline decoration-slate-300 hover:text-yellow-600 hover:decoration-yellow-600 transition-colors">เลือกวัสดุ (Silver/Gold/Platinum)</Link> และชนิดเพชรที่ต้องการ พร้อมพูดคุยเรื่องดีไซน์
        </span>
      ),
    },
    {
    ...
    },
    {
      ...
      description: "เมื่อผลิตเสร็จ ทางร้านจะส่งรูปภาพและวิดีโอคมชัดให้ตรวจสอบความเรียบร้อย ก่อนชำระยอด 50% ที่เหลือ",
    ...
    },
    {
      ...
      description: "เลือกช่องทางจัดส่งได้ตามสะดวก ทั้ง EMS ทั่วไทย (ฟรี) หรือ Messenger ส่งด่วนใน กทม.",
  ...
  return (
    <div className="min-h-screen bg-[#FDFBF7] font-sans selection:bg-yellow-100 pb-6 relative">
      ...
      <section className="relative h-[861px] flex items-center justify-center overflow-hidden">
        ...
        <div className="max-w-3xl mx-auto relative z-20 text-center px-4 mt-16">
          <Reveal immediate={true}>
            <span className="text-xs font-bold tracking-widest text-yellow-400 uppercase mb-4 block drop-shadow-md">Custom Made Jewelry</span>
          </Reveal>
            ...
            {steps.map((step, index) => (
              <div key={step.id} className={`relative flex flex-col md:flex-row items-center gap-8 ${index % 2 === 0 ? 'md:flex-row-reverse' : ''}`}>
                ...
                <div className="w-full md:w-1/2 px-4 text-center md:text-left">
                  <Reveal delay={0.2} className={`${index % 2 === 0 ? 'md:text-left' : 'md:text-right'}`} immediate={index === 0}>
                    <span className="inline-block md:hidden bg-slate-800 text-white text-[18px] px-6 py-2 rounded-full font-bold tracking-wider mb-3">ขั้นตอนที่ {step.id}</span>
                    <h3 className="text-xl font-serif text-slate-800 mb-1">{step.title}</h3>
                    <span className="text-xs text-slate-400 uppercase tracking-wider block mb-4">{step.engTitle}</span>
                    <p className="text-sm text-slate-600 font-light leading-relaxed mb-4">
                      {step.description}
                    </p>
                    <div className={`flex flex-col gap-2 ${index % 2 === 0 ? 'md:items-start' : 'md:items-end'} items-center`}>
                      {step.tag && (
                ...
                <div className="w-full md:w-1/2 px-4">
                  <Reveal delay={0.4} immediate={index === 0}>
                    <div className={`aspect-[4/3] rounded-2xl bg-white border border-slate-100 flex items-center justify-center relative overflow-hidden group shadow-sm hover:shadow-md transition-all duration-500`}>
      ...
      <section className="bg-white mt-10  border-t border-slate-100 relative z-10">
        <div className="container mx-auto px-4 max-w-4xl">
          <Reveal>
            <h2 className="text-xl font-serif text-slate-800 text-center mt-10 mb-10">มาตรฐานที่คุณมั่นใจได้</h2>
          </Reveal>
            ...
            links={[
              {
                ...
                description: 'เปรียบเทียบวัสดุในการทำจิวเวลรี่ ตั้งแต่ทอง 9k, 14k, 18k และ Platinum',
